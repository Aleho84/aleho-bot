...
const app = express();
app.set('trust proxy', 1); // Trust first, necessary for secure cookies behind Nginx
let httpServer;
...
connectDB()
    .then(async () => {
        // Dynamic import to avoid circular dependency issues with DAOs initialization
        const { seedAdmin } = await import('./config/adminSeeder.js');
        await seedAdmin();
