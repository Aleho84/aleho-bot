...
const userGeminiInstances = new Map();
...
const commandHandler = async (bot, chatUser, text) => {
  ...
  switch (botCmd) {
    ...
    case "resetcontext":
      if (userGeminiInstances.has(chatUser.chatID)) {
        userGeminiInstances.get(chatUser.chatID).resetChat();
        bot.sendMessage(
          chatUser.chatID,
        ...
        bot.sendMessage(
          chatUser.chatID,
          `🤔 No hay contexto para olvidar, ¡empecemos de nuevo!`
        );
      }
      break;
  ...
  switch (text) {
    ...
    default:
      try {
        ...
        let geminiAIInstance = userGeminiInstances.get(chatUser.chatID);
        if (!geminiAIInstance) {
          geminiAIInstance = new GeminiIA(constant.GEMINI_API_KEY);
          userGeminiInstances.set(chatUser.chatID, geminiAIInstance);
        }

        const botResponse = await geminiAIInstance.chat(text || "");
        ...
        // No reinstanciar globalmente, solo para el usuario si es necesario
        ...
        // Si hay un error grave con la instancia de Gemini para este usuario, la eliminamos para que se cree una nueva en la próxima interacción.
        userGeminiInstances.delete(chatUser.chatID);
      }
      break;
  }
};
